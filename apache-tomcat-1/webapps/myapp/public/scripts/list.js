...
$.ajax({
    ...
    for (const item of json) {
        ...
        let a = `<a href="product.html?search=${searchBar.value}&id=${item.prdId}">\
                    <img src=data:image/jpg;base64,${item.img}></img>\
                    <h6 class="heading">${item.prdName}</h6>\
                    <h6 class="heading">&#8361;${item.curPrice}</h6>\
                    <hr><p>${item.isBidding?'now on bidding':''}</p>\
                    <h6 class="heading">${item.place}</h6>\
                </a>`;
